...
int Client::Conntoserver()
{
    ...
    if(setsockopt(this->sockfd, SOL_SOCKET, SO_LINGER, &lo, sizeof(lo)) != 0)
    {
    ...
    serveraddr.sin_port = ntohs(this->serverport);
    inet_pton(AF_INET, this->serverip.c_str(), &serveraddr.sin_addr);

    //connect to server

    if(connect(this->sockfd, (sockaddr *) &serveraddr, sizeof(serveraddr))==-1)
    {
...
int Client::Sendmsg()
{
    ...
    cout << " >";
    getline(cin, s);
    int bytessent = send(this->sockfd, s.c_str(), sizeof(s.c_str()), 0);
    if(bytessent == -1)
    {
